...
def get_f1_scores(total_err_scores, gt_labels, topk=1):
    ...
    for i, indexs in enumerate(topk_indices):
        sum_score = sum(
            score
            for score in sorted([total_err_scores[index, i] for index in indexs])
        )
...
def get_val_performance_data(total_err_scores, normal_scores, gt_labels, topk=1, save_path=None):
    total_features = total_err_scores.shape[0]
    topk_indices = np.argpartition(
        total_err_scores, range(total_features - topk - 1, total_features), axis=0
    )[-topk:]
    ...
    if save_path:
        ...
        np.save(f"{save_path}test_scores.npy", total_topk_err_scores)
